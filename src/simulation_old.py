from person_old import Person, Couple, AVAILABLE_GENERATIONS, ALL_GENERATIONS, MALE, FEMALE
class Simulation:
    ...
    def _select(hunter : list[Person],deer : list[Person]) -> list[Couple]:#backtracking
        ...
        max_h = len(hunter)
        def BackTrack(idx_h : int) -> bool:
            if(idx_h == max_h):
                return True
            h : Person = hunter[idx_h]
            for d in deer:
                if h == d:
                    continue
                if h.is_appropriate_person(d):
                    h.has_mingled = True
                    d.has_mingled = True
                    if BackTrack(idx_h + 1):
                        couples_list.append(Couple(h,d))
                        return True
                    else:
                        h.has_mingled = False
                        d.has_mingled = False              
        ...
        if BackTrack(0):
            ...
            print("BackTrack failed returning empty list...")
            return []
